...
|---|---|---|
| Live database, `sorted/`, index | full-disk encryption only (FileVault, **On** at 2026-09-22) | staff app is loopback only; portal is HTTPS through the tunnel, `Secure` cookies |
...
| Erasure key | `~/.clinic-erasure.key` (or `CLINIC_ERASURE_KEY_FILE`), mode 600, created on the first erasure | matching a restored patient to a tombstone without storing their code |
...
id, but not a patient who was re-created under a new id. Keep a second copy of each somewhere safe
and offline - that is the owner's job, not a script's.
...
`backups/pre-p52-tz-20260916T213905Z.sqlite`; an encrypted copy is at
`backups/migration/clinic-pre-p52-tz-20260916T213905Z.cbk` and matches it table for table. The
plaintext file should be deleted by the owner.
